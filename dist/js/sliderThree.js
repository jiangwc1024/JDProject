(function () {
    ...
    SliderThree.prototype.createDom = function() {
        var oUl = $('<ul class="imgListThree"></ul>');
        var Spot = $('<div class="spotThree"></div>')
        ...
        var leftBtn = $('<div class="left-btnThree">&lt</div>');
        var rightBtn = $('<div class="right-btnThree">&gt</div>');
        this.wrap.append(oUl).append(leftBtn).append(rightBtn).append(Spot);
    }

    SliderThree.prototype.initStyle = function() {
        $('*', this.wrap).css({
            margin: 0,
            padding: 0,
            listStyle: 'none',
        ...
        $('.imgListThree', this.wrap).css({
            width: this.width,
            height: this.height,
            position: 'relative',
        });
        $('.imgListThree li', this.wrap).css({
            width: this.width,
            height: this.height,
            position: 'absolute',
            left: 0,
        ...
        $('.imgListThree li a, .imgListThree li a img', this.wrap).css({
            display: 'inline-block',
            width: this.width,
            height: this.height,
        });
        $('.left-btnThree, .right-btnThree', this.wrap).css({
            width: this.btnWidth,
        ...
        $('.right-btnThree', this.wrap).css({
            right: 0,
        });
        $('.spotThree', this.wrap).css({
    ...
    SliderThree.prototype.bindEvent = function() {
        var self = this;
        $('.left-btnThree', this.wrap).click(function() {
            self.move('prev');
        });
        $('.right-btnThree', this.wrap).click(function() {
            self.move('next');
        });
        $('.spotThree span').click(function(e) {
    ...
    SliderThree.prototype.move = function(dir) {
        if(this.flag) {
            return false;
        }
        this.flag = true;
        ...
        var self = this;
        $('.imgListThree li', this.wrap).fadeOut().eq(this.nowIndex).fadeIn(function() {
            self.flag = false;
        });
        $('.spotThree  span', this.wrap).css({
            backgroundColor: this.spanColor,
    ...
    SliderThree.prototype.autoMove = function() {
        var self = this;
        this.time = setInterval(function() {
            if(this.direction == 'left') {
                $('.left-btnThree', this.wrap).trigger('click');
            }else{
                $('.right-btnThree', this.wrap).trigger('click');
            }
        },4000)
    }

    $.fn.extend({
        sliderThree: function(option) {
            option.wrap = this;
            new SliderThree(option);
        }
    })
} ())